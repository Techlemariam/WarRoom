...
if ($DryRun) {
  Write-Info "[DRY RUN] Would merge $script:BRANCH to develop and push"
} else {
  # Merge to develop
...
if ($STAGING_URL -and -not $DryRun -and -not $SkipStagingCheck) {
  Write-Step "3" "Staging Health Check"
...
Write-Step "4" "Pull Request (develop to main)"
