...
export async function GET() {
  try {
    ...
    // 2. Normalize Snyk Score (High entropy = High score)
    const securityScore = Number(((100 - snykMetrics.score) / 50).toFixed(1));
    ...
    const remediations = await generatePrescriptions(auditData, snykMetrics, tokenMetrics, tokenBurnSummary);
