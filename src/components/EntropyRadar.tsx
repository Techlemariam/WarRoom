...
export default function EntropyRadar() {
  ...
  if (loading) return (
    <div className="h-64 card-professional flex flex-col items-center justify-center gap-3">
      ...
      <div className="text-[11px] font-semibold uppercase tracking-wider text-on-surface-variant/40">Aggregating Audit Vectors...</div>
    </div>
  );
    ...
    <div className="card-professional shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-6">
        <div className="flex justify-between items-end">
          <div>
            <div className="text-[10px] font-bold text-on-surface-variant/50 uppercase mb-1">Confidence Score</div>
            <div className="flex items-baseline gap-1.5">
              <span className={`text-3xl font-bold ${isHealthy ? "text-on-surface" : "text-error"}`}>
                {totalScore.toFixed(1)}
              </span>
              <span className="text-xs text-on-surface-variant/40 font-medium">/ 10.0</span>
            </div>
          </div>
          <div className={`px-2.5 py-1 text-[10px] font-bold uppercase rounded border ${
            isHealthy 
      ...
      <div className="p-3 bg-surface-container-low border-t border-outline-variant grid grid-cols-2 gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-on-surface-variant/60">Target Threshold: <strong>{target}</strong></span>
        </div>
        <div className="text-right text-[10px] text-on-surface-variant/60">
          Variance: <strong className={isHealthy ? "text-primary" : "text-error"}>
            {isHealthy ? `-${(target - totalScore).toFixed(1)}` : `+${(totalScore - target).toFixed(1)}`}
          </strong>
        </div>
      </div>
    </div>
