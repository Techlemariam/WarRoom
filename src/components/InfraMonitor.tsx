...
import { Server, Database, AlertCircle, Activity } from "lucide-react";
      ...
      <div className="card-professional p-4 shadow-sm">
        <div className="flex justify-between items-center mb-4 pb-3 border-b border-outline-variant">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-on-surface">{hetzner?.name || 'Production Node'}</span>
          </div>
          <div className={`px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase ${
            hetzner?.status === 'running' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700'
          }`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <div className="w-full bg-surface-container h-1.5 rounded-sm">
              <div className="bg-primary h-full transition-all duration-1000 rounded-sm" style={{ width: `${cpuUsage}%` }}></div>
            </div>
          </div>
          <div className="space-y-1.5">
            <div className="flex justify-between text-[11px]">
              <span className="text-on-surface-variant font-medium">RAM Utilization</span>
              <span className="text-on-surface font-bold font-mono">{(memUsageRaw / (1024**3)).toFixed(1)} GB</span>
            </div>
            <div className="w-full bg-surface-container h-1.5 rounded-sm">
              <div className="bg-secondary h-full transition-all duration-1000 rounded-sm" style={{ width: `${memUsagePercent}%` }}></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card-professional overflow-hidden">
        <div className="p-3 bg-surface-container-low border-b border-outline-variant flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold uppercase tracking-wide text-on-surface-variant">Active Services</span>
          </div>
        ...
        <div className="max-h-[250px] overflow-y-auto">
          <div className="divide-y divide-outline-variant">
            {coolify?.apps?.map((app: any) => (
              <div key={app.uuid} className="p-3 flex items-center justify-between hover:bg-surface-container-low transition-colors">
                ...
                <span className="text-[10px] text-on-surface-variant/60 font-mono">{app.status?.split('(')[0] || 'unknown'}</span>
              </div>
            ))}
            {(!coolify?.apps || coolify.apps.length === 0) && (
              <div className="p-6 text-[11px] text-on-surface-variant/40 italic text-center">No active deployments detected.</div>
            )}
          </div>
        </div>
      </div>
