...
import { AlertCircle, Briefcase, CheckCircle2, ChevronRight, Clock } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {projects.map((project) => {
        const repoAudit = auditData?.vectors?.find((v: any) =>
          v.findings?.some((f: string) => f.includes(project.name))
        );
        ...
        return (
          <div
            key={project.name}
            className="card-professional flex flex-col hover:shadow-md transition-shadow"
          >
            ...
            <div className="p-4 flex-1 space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-on-surface-variant/40 uppercase">
                  Environment
                </label>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  ...
                  {(!project.active?.tasks || project.active?.tasks.length === 0) && (
                    <div className="text-[10px] text-on-surface-variant/30 italic">
                      No scheduled tasks.
                    </div>
                  )}
                </div>
            ...
            <div className="p-3 bg-surface-container-low border-t border-outline-variant flex justify-between items-center">
              ...
              <div className="text-on-surface font-bold text-[11px]">
                {project.revenue || '0'}{' '}
                <span className="text-[9px] font-medium opacity-40">SEK</span>
              </div>
            </div>
          </div>
        );
